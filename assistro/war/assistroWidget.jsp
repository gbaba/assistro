...
<%@ page import="com.vchat.util.AssistroWidget"%>
<%@ page import="com.vchat.beans.WidgetAgents"%>
...
<%
	...
	if (wc.getUser().getMaxAgent() == 1 && wc.getCount() > 10
			&& wc.getUser().getBusinessPlan().equals("Flying Solo")
			|| !wc.getUser().isActive()) {
		...
		if (wc.getWc() != null) {
			...
			if (wc.getWc().isProActiveChat()) {
				String close = (String) session
						.getAttribute("closeEve");
				if (close != null) {
					str = str
							.replace(
					...
					str = str
							...
							.replace(
									"expanded.setAttribute ('style','display:none');",
									"expanded.setAttribute ('style','display:block');");
					if(wc.getWc().getProOperator()!=null){
						str = str
								.replace("HelpDesk: ",
										wc.getWc().getProOperator()+": ");
					}
				}
			}
			if (!wc.getWc().isProActiveChat()) {
			...
			if (wc.getWc().getmPanel() != null
				...
				str = str.replace("Welcome_To_Assistro",
						wc.getWc().getmPanel()).replace(
						"Agent_Designation", wc.getWc().getmPanel());
			}
